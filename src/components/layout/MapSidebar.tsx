    ...
    return [
      {
        id: '1',
        type: 'login',
        user: { name: 'PixelAdventurer', dataAiHint: 'avatar user' },
        timestamp: new Date('2024-01-01T12:00:00Z'),
      },
      {
        id: '2',
        type: 'purchase',
        user: { name: 'ArtCollector7', dataAiHint: 'avatar user' },
        timestamp: new Date('2024-01-01T11:48:00Z'),
        details: 'Pixel (12,34) em Lisboa',
        region: 'Lisboa',
      },
      {
        id: '3',
        type: 'color_change',
        user: { name: 'ColorMaster', dataAiHint: 'avatar user' },
        timestamp: new Date('2024-01-01T11:35:00Z'),
        details: 'Pixel (5,8) para #FF0000',
        region: 'Porto',
      },
      {
        id: '4',
        type: 'achievement',
        user: { name: 'PixelAdventurer', dataAiHint: 'avatar user' },
        timestamp: new Date('2024-01-01T11:20:00Z'),
        details: 'Conquista: Primeiro Pixel!',
      },
    ];
  }
...
const initialStats: StatItem[] = [
  {
    ...
    value: 137,
    icon: <Users2 className="h-4 w-4 text-green-400" />,
    trend: 'neutral',
    tooltip: 'Utilizadores atualmente na plataforma.',
  },
  {
    ...
    trend: 'up',
    trendValue: '+5%',
    tooltip: 'Total de pixels que já foram adquiridos.',
  },
  {
    label: 'Pixels Livres',
    value: 8745723,
    ...
    trend: 'down',
    trendValue: '-0.2%',
    tooltip: 'Total de pixels ainda disponíveis para compra.',
  },
];
...
const FormattedTimestamp: React.FC<{ timestamp: Date }> = ({ timestamp }) => {
  ...
  return (
    <p className="font-code text-xs text-muted-foreground/80">{isClient ? timeString : '--:--'}</p>
  );
};
...
export default function MapSidebar() {
  ...
  useEffect(() => {
    const activityInterval = setInterval(() => {
      const randomActivityTypes: ActivityItem['type'][] = [
        'login',
        'purchase',
        'color_change',
        'view',
      ...
      const newActivity: ActivityItem = {
        ...
        timestamp: new Date(),
        details:
          Math.random() > 0.3 ? `Detalhe aleatório ${Math.floor(Math.random() * 100)}` : undefined,
      };
    ...
    const statsInterval = setInterval(() => {
      setStats(prevStats =>
        prevStats.map(stat => {
          ...
          const newValue =
            stat.label === 'Utilizadores Online'
      ...
      <SidebarHeader>
        <div className="flex items-center space-x-2">
          ...
          <div className="relative group-data-[collapsible=icon]:hidden">
            ...
            <div className="absolute -right-1 -top-1 h-2 w-2 animate-ping rounded-full bg-green-400" />
          </div>
        </div>
        <p className="text-gradient-gold font-headline text-lg group-data-[collapsible=icon]:hidden">
          Painel de Controlo
        </p>
      </SidebarHeader>
      <SidebarContent className="animate-fade-in animation-delay-200"></SidebarContent>
    </Sidebar>
  );
}
