...
export default function BottomNavBar() {
  ...
  // Daily reward system
  useEffect(() => {
    const lastRewardDate = localStorage.getItem('last-daily-reward');
    const today = new Date().toDateString();
    
    if (lastRewardDate !== today && user) {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: "Recebeu 50 créditos por entrar hoje. Volte amanhã para mais!",
        });
        localStorage.setItem('last-daily-reward', today);
        setDailyRewardClaimed(true);
      }, 2000);
    }
  }, [user, addCredits, toast]);
              ...
              >
                ...
                {link.badge && notifications > 0 && link.href === "/member" && ( 
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500, damping: 15 }}>
                    <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 text-xs bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 flex items-center justify-center animate-heartbeat shadow-lg border border-background">
                ...
                {link.href === "/" && dailyRewardClaimed && (
                  ...
                  >
                    <div className="w-3 h-3 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full animate-pulse border border-background" />
                  </motion.div>
                )}
        ...
        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: -8, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20, delay: 0.3 }} className="absolute -top-8 left-1/2 transform -translate-x-1/2 z-10">
            ...
            <>
              <DropdownMenuItem onSelect={(e) => e.preventDefault()} className="cursor-pointer hover:bg-primary/10 transition-colors group">
                <div className="flex items-center w-full"> 
                  ...
                  <div>
                    <div className="font-medium">Recompensa Diária</div>
                    <div className="text-xs text-muted-foreground">50 créditos grátis disponíveis!</div>
                  </div>
                  <Badge className="ml-auto bg-yellow-500 text-black text-xs animate-pulse">NOVO</Badge>
                </div>
              </DropdownMenuItem>
              <DropdownMenuSeparator className="bg-primary/10" />
            </>
          ...
          <DropdownMenuItem className="cursor-default">
            <div className="flex items-center justify-between w-full text-xs"> 
              <span>Créditos: {credits.toLocaleString()}</span>
              <span>Pixels: {42}</span>
            </div>
          </DropdownMenuItem>
