...
export default function SwipeGestures({
  ...
  const handleDragEnd = (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    const isSignificantSwipe = absX > 50 || absY > 50 || 
  ...
  const renderSwipeIndicator = (direction: 'left' | 'right' | 'up' | 'down') => {
    ...
    return (
      <motion.div
        key={direction}
        className={`absolute ${positions[direction]} z-10 pointer-events-none`}
      ...
      {!disabled && (
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-2 left-2 right-2">
            <div className="bg-black/60 backdrop-blur-sm text-white text-xs p-2 rounded-lg text-center">
