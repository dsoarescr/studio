  ...
  const generateSecret = async () => {
    ...
    try {
      ...
      const keys = Array.from({ length: 8 }, () =>
        Array.from({ length: 4 }, () => Math.random().toString(36).substring(2, 6)).join('-')
      );
      ...
      {step === 'intro' && (
        <div className="rounded-lg border bg-card p-6">
          ...
          <CardFooter className="flex justify-end border-t pt-4">
            <Button onClick={() => setStep('setup')}>Configurar 2FA</Button>
          </CardFooter>
        </div>
      )}
                ...
                <div className="space-y-2">
                  ...
                  {secret && (
                    <div className="mt-2 flex items-center">
                      <Input value={secret} readOnly className="text-center font-mono" />
                      <Button
                        variant="outline"
                        size="icon"
                        className="ml-2"
                  ...
                  <div className="rounded-lg bg-muted/30 p-3">
                    <div className="grid grid-cols-2 gap-2">
                      {recoveryKeys.map((key, index) => (
                        <div
                          key={index}
                          className="rounded bg-background/50 p-1 text-center font-mono text-xs"
                        >
                          {key}
                        </div>
                      ))}
        ...
        <div className="rounded-lg border bg-card p-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-start gap-2 rounded-lg bg-muted/30 p-3">
              ...
              <p className="text-sm text-muted-foreground">
                Abra a sua aplicação de autenticação e insira o código de 6 dígitos mostrado para a
                conta Pixel Universe.
              </p>
            </div>
          </CardContent>
          <CardFooter className="flex justify-between border-t pt-4">
          ...
          <CardContent className="space-y-4">
            <div className="rounded-lg bg-green-500/10 p-4 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                A partir de agora, será necessário inserir um código de verificação sempre que
                iniciar sessão.
              </p>
            </div>
            ...
            <div className="flex items-start gap-2 rounded-lg bg-amber-500/10 p-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Certifique-se de que guardou as suas chaves de recuperação num local seguro. Elas
                  são a única forma de recuperar a sua conta se perder acesso ao seu dispositivo de
                  autenticação.
                </p>
              </div>
            </div>
