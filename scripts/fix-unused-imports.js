...
function removeUnusedImport(content, importName) {
  ...
  content = content.replace(new RegExp(`(import\\s*{)\\s*${importName}\\s*,([^}]*})`, 'g'), '$1$2');
  content = content.replace(new RegExp(`(import\\s*{)\\s*${importName}\\s*(})`, 'g'), '');
...
function fixSpecificIssues(content) {
  ...
  // Fix: no-this-alias
  content = content.replace(
    /const\s+context\s*=\s*this;/g,
    '// const context = this; // Fixed: use arrow function instead'
  );
