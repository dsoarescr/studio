...
const SAFE_REMOVALS = {
  ...
  removeFromImports: [
    ...
    { file: /SecurityDashboard\.tsx$/, imports: ['CardFooter', 'Key', 'Smartphone', 'Eye', 'EyeOff', 'RefreshCw', 'User', 'Mail', 'CreditCard'] },
    { file: /TwoFactorAuth\.tsx$/, imports: ['Card', 'Separator', 'RefreshCw'] },
    { file: /HelpCenter\.tsx$/, imports: ['Separator', 'Avatar', 'AvatarFallback', 'AvatarImage', 'motion'] },
    { file: /PixelStories\.tsx$/, imports: ['Card', 'CardContent', 'Progress'] },
    { file: /PixelAR\.tsx$/, imports: ['CardHeader', 'CardTitle', 'Smartphone', 'Download'] },
    { file: /PixelLiveStream\.tsx$/, imports: ['Users', 'Crown', 'Coins', 'Zap', 'Wifi', 'Signal', 'Volume2', 'Settings', 'cn'] },
  ],
  
  // Remover linhas de declaração de variáveis específicas
  removeVariableDeclarations: [
    { file: /.*\.tsx?$/, pattern: /const\s+\w+\s*=\s*\w+;\s*\/\/.*is assigned a value but never used/ },
    { file: /.*\.tsx?$/, pattern: /let\s+\w+\s*=\s*\w+;\s*\/\/.*is assigned a value but never used/ },
  ]
};
